...
ann_data |>
  ...
  annotate(geom = "text", x = 2008, y = 73, label = "China",
           colour = sg_colour_values["dark-blue"],
           hjust = 0, vjust = 0.5) +
  annotate(geom = "text", x = 2008, y = 79.4, label = "United Kingdom",
           colour = sg_colour_values["orange"],
           hjust = 0, vjust = 0.5) +
  labs(
    x = "Year",
    y = NULL,
...
The following examples use `geom_text()`, however `geom_label()` is also an option and draws a rectangle behind the text, sometimes making it easier to read. These functions have `nudge` arguments that can be used to displace text to improve the positioning.
...
Annotations may also be used to add value labels to a bar chart.
...
ggplot(bar_data, aes(x = reorder(country, -lifeExp), y = lifeExp)) +
  ...
  geom_text(aes(label = round(lifeExp, 1)), nudge_y = -5, colour = "white") +
  theme_sg() +
...
**Note**: The `annotate()` function should be used to add annotations with manually defined positioning co-ordinates, whereas `geom_text()` should be used when using co-ordinates defined in a data frame. Although the reverse may work, text can appear blurry.
